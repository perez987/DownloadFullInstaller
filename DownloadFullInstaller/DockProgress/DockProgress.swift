...
public enum DockProgress {
    ...
    /**
     ...
     ```
     */
    public weak static var progressInstance: Progress? {
        didSet {
...
public extension DockProgress {
    ...
    enum Style: Sendable {
         ...
         */
        case badge(color: Color = .accentColor, badgeValue: @MainActor @Sendable () -> Int)
         ...
         */
        case customView(@MainActor @Sendable (_ progress: Double) -> any View)
         ...
         */
        case customCanvas(@MainActor @Sendable (_ context: GraphicsContext, _ size: CGSize, _ progress: Double) -> Void)
...
extension DockProgress {
    private final class ContentView: NSView {
        ...
        override func draw(_: CGRect) {
            ...
            case let .customView(viewProvider):
                updateHostingView(with: viewProvider(displayedProgress))
            case let .customCanvas(canvasRenderer):
